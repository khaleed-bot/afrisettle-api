...
function explorerUrl() {
  ...
  const value = invoice?.txHash || invoice?.depositAddress || invoice?.walletAddress;
...
function createQrSvg(value) {
  ...
  return qr
    .createSvgTag({ cellSize: 6, margin: 24, scalable: true })
    .replace(
      "<svg",
      '<svg class="h-[220px] w-[220px]" role="img" aria-label="Payment address QR code"'
    )
    .replace("<rect", '<rect shape-rendering="crispEdges"');
}

function paymentAddress() {
  return invoice?.depositAddress || invoice?.walletAddress || "";
}

function renderQr(address) {
  if (!address) {
    qrCode.innerHTML =
      "<span class='text-center text-xs font-bold text-slate-500'>Address unavailable</span>";
    return;
  }

  qrCode.innerHTML = createQrSvg(address.trim());
  qrCode.dataset.encodedValue = address.trim();
}
...
function render() {
  ...
  depositAddress.textContent = address || "Deposit address is not available yet.";

  tagBlock.classList.toggle("hidden", !invoice.depositAddressTag);
  depositAddressTag.textContent = invoice.depositAddressTag || "";
  explorerLink.href = explorerUrl();
  explorerLink.classList.toggle("pointer-events-none", !address && !invoice.txHash);
  explorerLink.classList.toggle("opacity-50", !address && !invoice.txHash);
